...
版本：v1.1；日期：2026-09-20；状态：E2 对话与 E3 动作页面已实现，E4 真实写入待实施。
...
| E4 | 接入 `ActionProposalCard`、真实提案和受控任务执行；复验批准内容、幂等和回读 | 用户能区分提案、待批准、执行中、未知和成功 |
| E5 | 完成企业登录、会话撤销、CSRF、数据保留、可访问性、响应式布局、观测和发布锁定 | 指定真实用户和数据范围满足试运行门槛 |
