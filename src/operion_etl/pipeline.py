...
ERPNEXT_SUPPLIER_HEADERS = [
    "ID", "Supplier Name", "Supplier Type", "Supplier Group", "Operion Source Key",
]
ERPNEXT_ITEM_HEADERS = [
    "ID", "Item Code", "Item Name", "Item Group", "Default Unit of Measure",
...
ERPNEXT_SALES_ORDER_HEADERS = [
    ...
    "Price List Exchange Rate", "Customer's Purchase Order", "Operion Source Key",
    "WWI Source Status", "Item Code (Items)", "Item Name (Items)",
    "Delivery Date (Items)", "Quantity (Items)", "Stock UOM (Items)", "UOM (Items)",
    "UOM Conversion Factor (Items)", "Rate (Items)", "Amount (Items)",
    "Basic Rate (Company Currency) (Items)", "Amount (Company Currency) (Items)",
    "Operion Source Key (Items)",
]
ERPNEXT_PURCHASE_ORDER_HEADERS = [
    "ID", "Series", "Supplier", "Date", "Required By", "Company", "Currency",
    "Exchange Rate", "Price List", "Price List Currency", "Price List Exchange Rate",
    "Operion Source Key", "WWI Supplier Reference", "WWI Source Status",
    "Item Code (Items)", "Item Name (Items)", "Required By (Items)",
    "Quantity (Items)", "Stock UOM (Items)", "UOM (Items)",
    "UOM Conversion Factor (Items)", "Rate (Items)", "Amount (Items)",
    "Rate (Company Currency) (Items)", "Amount (Company Currency) (Items)",
    "Operion Source Key (Items)",
]
...
""")
    ...
    purchase_lines = db.query_json(f"""
...
""") if purchase_orders else []
    all_item_ids = sorted(
        {int(row["StockItemID"]) for row in sales_lines + purchase_lines if row.get("StockItemID") is not None}
    )
...
""")
    supplier_ids_set = {int(row["SupplierID"]) for row in purchase_orders if row.get("SupplierID")}
    supplier_ids_set.update(int(row["SupplierID"]) for row in products if row.get("SupplierID"))
...
""")
    return {"table_counts": {row["source_table"]: row["record_count"] for row in counts}, **ranges[0]}
    ...
    for role, rows, id_key, name_key in (
        ...
        for row in rows:
            ...
            orgs.append({
                ...
                "name": row[name_key],
                "roles": role,
                "primary_contact_source_id": row.get("PrimaryContactPersonID") or "",
                ...
                "credit_limit": _money(row.get("CreditLimit")) if role == "customer" else "",
                "created_at": row.get("AccountOpenedDate") or row.get("CreationDate") or "",
...
def _validate(canonical: dict[str, list[dict]]) -> list[dict]:
    errors: list[dict] = []
    keys = {name: {row["canonical_id"] for row in rows} for name, rows in canonical.items()}

    def require(entity: str, row: dict, fields: tuple[str, ...]) -> None:
        for field in fields:
            if row.get(field) in (None, ""):
                errors.append({
                    "entity": entity, "source_id": row.get("source_id", ""),
                    "rule": "required", "field": field, "error": "value is required",
                })
    ...
    for row in canonical["products"]:
        ...
        if Decimal(row["units_per_outer"]) <= 0:
            errors.append({"entity": "product", "source_id": row["source_id"], "rule": "positive", "field": "units_per_outer", "error": "must be positive"})
    for row in canonical["sales_orders"]:
        require("sales_order", row, ("canonical_id", "customer_canonical_id", "order_date"))
        if row["customer_canonical_id"] not in keys["organizations"]:
            errors.append({"entity": "sales_order", "source_id": row["source_id"], "rule": "reference", "field": "customer_canonical_id", "error": "customer not extracted"})
    for row in canonical["sales_order_lines"]:
        require("sales_order_line", row, ("canonical_id", "sales_order_canonical_id", "product_canonical_id", "uom"))
        if row["sales_order_canonical_id"] not in keys["sales_orders"] or row["product_canonical_id"] not in keys["products"]:
            errors.append({"entity": "sales_order_line", "source_id": row["source_id"], "rule": "reference", "field": "parent/product", "error": "reference not extracted"})
        expected = Decimal(row["ordered_quantity"]) * Decimal(row["unit_price_ex_tax"])
        if abs(expected - Decimal(row["net_amount"])) > Decimal("0.01"):
            errors.append({"entity": "sales_order_line", "source_id": row["source_id"], "rule": "amount", "field": "net_amount", "error": "quantity × unit price mismatch"})
    for row in canonical["purchase_orders"]:
        if row["supplier_canonical_id"] not in keys["organizations"]:
            errors.append({"entity": "purchase_order", "source_id": row["source_id"], "rule": "reference", "field": "supplier_canonical_id", "error": "supplier not extracted"})
    for row in canonical["purchase_order_lines"]:
        if row["purchase_order_canonical_id"] not in keys["purchase_orders"] or row["product_canonical_id"] not in keys["products"]:
            errors.append({"entity": "purchase_order_line", "source_id": row["source_id"], "rule": "reference", "field": "parent/product", "error": "reference not extracted"})
    return errors
...
def _scenarios(canonical: dict[str, list[dict]]) -> list[dict]:
    ...
    if not lines:
        return []
    order_date = max(date.fromisoformat(row["order_date"]) for row in canonical["sales_orders"])
    ...
    for index, (case_id, kind, expected) in enumerate(definitions):
        ...
        inbound_date = ""
        if case_id == "F02":
            on_hand, inbound, inbound_date = demand - 2, Decimal(2), (deadline - timedelta(days=1)).isoformat()
        elif case_id == "F03":
            on_hand, inbound, inbound_date = demand - 2, Decimal(2), (deadline + timedelta(days=1)).isoformat()
        elif case_id == "F04":
...
def _erpnext_contact_rows(
    contacts: list[dict], organizations_by_id: dict[str, dict],
) -> list[dict]:
    rows: list[dict] = []
    for contact in contacts:
        name_parts = contact["full_name"].strip().split(maxsplit=1)
        ...
        link_name = ""
        if organization:
            link_doctype = "Customer" if organization["roles"] == "customer" else "Supplier"
...
def _erpnext_sales_order_rows(
    orders: list[dict], lines: list[dict], organizations_by_id: dict[str, dict],
    ...
    for order in orders:
        ...
        for index, line in enumerate(order_lines):
            ...
            parent = {
                "ID": "",
                "Series": "SAL-ORD-.YYYY.-",
                "Customer": organizations_by_id[order["customer_canonical_id"]]["name"],
                "Order Type": "Sales",
            ...
            } if index == 0 else _blank_parent(
                ERPNEXT_SALES_ORDER_HEADERS, "Item Code (Items)"
            )
            rows.append({
                **parent,
    ...
    for order in orders:
        ...
        for index, line in enumerate(order_lines):
            ...
            parent = {
                "ID": "",
                "Series": "PUR-ORD-.YYYY.-",
                "Supplier": organizations_by_id[order["supplier_canonical_id"]]["name"],
                "Date": order["order_date"],
                ...
                ERPNEXT_PURCHASE_ORDER_HEADERS, "Item Code (Items)"
            )
            rows.append({
                **parent,
                "Item Code (Items)": line["product_canonical_id"],
...
def _exports(canonical: dict[str, list[dict]]) -> dict[str, dict[str, list[dict]]]:
    customers = [row for row in canonical["organizations"] if row["roles"] == "customer"]
    suppliers = [row for row in canonical["organizations"] if row["roles"] == "supplier"]
    organizations_by_id = {row["canonical_id"]: row for row in canonical["organizations"]}
    ...
    erpnext = {
        ...
        "sales_orders": _erpnext_sales_order_rows(
            canonical["sales_orders"], canonical["sales_order_lines"],
            organizations_by_id, products_by_id,
        ),
        "purchase_orders": _erpnext_purchase_order_rows(
            canonical["purchase_orders"], canonical["purchase_order_lines"],
            organizations_by_id, products_by_id,
        ),
    }
    twenty = {
...
def _identity_map(canonical: dict[str, list[dict]], batch_id: str) -> list[dict]:
    entity_targets = {
        "organizations": ("erpnext", "twenty"), "contacts": ("erpnext", "twenty"),
        "products": ("erpnext",), "sales_orders": ("erpnext",),
        "sales_order_lines": ("erpnext",), "purchase_orders": ("erpnext",),
    ...
    for entity, targets in entity_targets.items():
        for record in canonical[entity]:
            for target in targets:
                rows.append({
                    "entity_type": entity.rstrip("s"), "source_system": "wwi",
                    "source_id": record["source_id"], "canonical_id": record["canonical_id"],
                    "target_system": target, "target_id": "", "batch_id": batch_id,
                    "mapping_version": MAPPING_VERSION, "load_status": "pending",
                    "error": "",
                })
    return rows
...
def run_etl(
    ...
    if not source_manifest_path.is_file() or not backup_path.is_file():
        raise FileNotFoundError("Snapshot must contain source_manifest.json and the WWI backup")
    ...
    roots = [data_root / "canonical" / batch_id, data_root / "exports" / "erpnext" / batch_id,
             data_root / "exports" / "twenty" / batch_id, data_root / "quarantine" / batch_id,
    ...
    manifest = {
        ...
        "selection": {"strategy": "latest sales orders plus recent purchase orders sharing selected products", "sales_order_limit": order_limit},
        "source_profile": source_profile,
        "record_counts": {name: len(rows) for name, rows in canonical.items()},
        "export_counts": {system: {name: len(rows) for name, rows in files.items()} for system, files in exports.items()},
        "identity_map_rows": len(identity),
        "validation": {"status": "passed" if not errors else "failed", "error_count": len(errors)},
    ...
    if errors:
        raise RuntimeError(f"ETL validation failed with {len(errors)} error(s); see {quarantine_dir}")
    return report_dir
